...
Gun::Gun(GunParams&& params) :
    ...
    gunParams(std::move(params)), dragging(false), arrowAim({20.f, 60.f}, 6, SpriteNames::arrow, 0)
{
...
void Gun::handleDrag(sf::Vector2f deltaDrag, bool isFinished) {
    if ((gunParams.primaryWeapon == nullptr || !gunParams.primaryWeapon->isDraggable) &&
        (gunParams.secondaryWeapon == nullptr || !gunParams.secondaryWeapon->isDraggable))
        return;
    ...
    if (gunParams.primaryWeapon->isDraggable)
        attackPrimary(velocityScalar);
    else if (gunParams.secondaryWeapon->isDraggable)
        attackPrimary(velocityScalar);
}
